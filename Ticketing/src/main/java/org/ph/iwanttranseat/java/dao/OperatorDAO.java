...
public class OperatorDAO {
	...
	private static final String UPDATE_PASSWORD = "UPDATE operator set operator_password = ?  where operator_username = ?;";
	...
	private static final String SELECT_OPERATOR_BY_USERNAME_AND_PASSWORD = "SELECT * FROM operator WHERE operator_username = ? AND operator_password = ?";
	...
	public OperatorModel selectOperator(long operatorId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				operatorModel = new OperatorModel(id, operator_firstname, operator_lastname, operator_username,
						operator_password);
			}
	...
	public List<OperatorModel> selectAllOperator() {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (rs.next()) {
				...
				String operator_username = rs.getString("operator_username");
				String operator_password = rs.getString("operator_password");
				todosOperator.add(new OperatorModel(id, operator_firstname, operator_lastname, operator_username,
						operator_password));
...
//	}
	...
	public boolean updateOperator(OperatorModel operatorModel) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
				PreparedStatement statement = connection.prepareStatement(UPDATE_OPERATOR);) {
	...
	public boolean validateOldPassword(String operatorUsername, String oldPassword) {
		...
		try (Connection connection = JDBCUtils.getConnection();
				...
				PreparedStatement preparedStatement = connection
						.prepareStatement(SELECT_OPERATOR_BY_USERNAME_AND_PASSWORD);) {
			preparedStatement.setString(1, operatorUsername);
	...
	public boolean changePassword(OperatorModel operatorModel) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
				PreparedStatement statement = connection.prepareStatement(UPDATE_PASSWORD);) {
			statement.setString(1, operatorModel.getOperator_password());
			statement.setString(2, operatorModel.getOperator_username());
