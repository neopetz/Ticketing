...
public class TravelScheduleDAO {
	...
	private static final String SELECT_All_TRAVEL_SCHEDULE = "SELECT travelId, "
			...
			+ "CONCAT(bp2.firstname, ' ',bp2.lastname) AS conductor, " + "fare " + "FROM travel_schedule "
	...
	public List<TravelScheduleModel> selectAllTravelSchedules() {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (resultSet.next()) {
				...
				listOfTravelSchedule
						.add(new TravelScheduleModel(travelId, travel_schedule, travel_bus, driver, conductor, fare));
				System.out.println(travelId + " " + travel_schedule + " " + travel_bus + " " + driver + " " + conductor
						+ " " + fare);
			}
	...
	public TravelScheduleModel selectTravelScheduleById(int travelId) {
		...
		try (Connection connection = JDBCUtils.getConnection();
			...
			while (resultSet.next()) {
				...
				String conductor = resultSet.getString("conductor");
				int fare = resultSet.getInt("fare");
				travelSchedule = new TravelScheduleModel(travel_id, travel_schedule, travel_bus, driver, conductor,
						fare);
				System.out.println(travel_id + " " + travel_schedule + " " + travel_bus + " " + driver + " " + conductor
						+ " " + fare);
			}
	...
	private static final String UPDATE_TRAVEL_SCHEDULE = "UPDATE travel_schedule SET sched_id = ?, bus_id = ?, driver_id = ?, conductor_id = ? WHERE travelId = ?;";

	public boolean updateTravelSchedule(TravelScheduleModel updatedTravelSchedule) throws SQLException {
		boolean rowUpdated;
		try (Connection connection = JDBCUtils.getConnection();
				PreparedStatement preparedStatement = connection.prepareStatement(UPDATE_TRAVEL_SCHEDULE);) {

			preparedStatement.setInt(1, updatedTravelSchedule.getSchedule_id());
			preparedStatement.setInt(2, updatedTravelSchedule.getBus_id());
			preparedStatement.setInt(3, updatedTravelSchedule.getDriver_id());
			preparedStatement.setInt(4, updatedTravelSchedule.getConductor_id());
			preparedStatement.setInt(5, updatedTravelSchedule.getTravelId());
			System.out.println(preparedStatement);
